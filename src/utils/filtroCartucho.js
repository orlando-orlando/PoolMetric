...
/* ================================================================
   HELPER — flujo efectivo según uso
   residencial → flujoResidencial (si > 0)
   cualquier otro uso → flujoComercial
   Si flujoResidencial === 0 el equipo no aplica para uso residencial
   ================================================================ */
...
/* ================================================================
   HIDRÁULICA — misma estructura que filtroArena y prefiltro
   1 m por tramo, 6.5 ft/s máx, 7 ft carga fija
   ================================================================ */
function calcularHidraulicaCartucho({ flujoPorFiltro, cantidad }) {
...
/* ================================================================
   SELECCIÓN ÓPTIMA
   Filtra por uso antes de comparar.
   flujoResidencial = 0 → el equipo no aplica para residencial.
   ================================================================ */
function seleccionarFiltroCartucho(flujoMaximo, usoGeneral) {
  const catalogo = filtrosCartucho
    .filter(f => f.metadata.activo)
    .map(f => ({ filtro: f, flujoEf: flujoEfectivo(f, usoGeneral) }))
    .filter(({ flujoEf }) => flujoEf !== null && flujoEf > 0);

  let mejor = null;

  for (const { filtro, flujoEf } of catalogo) {
    const cantidad   = Math.ceil(flujoMaximo / flujoEf);
    const flujoTotal = cantidad * flujoEf;
    const exceso     = flujoTotal - flujoMaximo;

    if (!mejor) { mejor = { filtro, flujoEf, cantidad, flujoTotal, exceso }; continue; }
...
/* ================================================================
   FUNCIÓN PRINCIPAL
   ================================================================ */
export function filtroCartucho(flujoMaximo, usoGeneral = "comercial") {
  ...
  if (!seleccion) {
    return { error: "Sin filtros de cartucho disponibles para este uso." };
  }

  const { filtro, flujoEf, cantidad } = seleccion;

  const hidraulica = calcularHidraulicaCartucho({ flujoPorFiltro: flujoEf, cantidad });
  ...
  return {
    seleccion: {
      ...
      cantidad,
      flujoEfectivo:   fix2(flujoEf),
      flujoTotal:      fix2(cantidad * flujoEf),
...
/* ================================================================
   FUNCIÓN MANUAL
   ================================================================ */
export function calcularCargaFiltroCartuchoManual(flujoEf, cantidad) {
  if (!flujoEf || flujoEf <= 0 || !cantidad || cantidad <= 0) {
    return { error: "Flujo o cantidad inválidos para cálculo manual." };
  }
  return calcularHidraulicaCartucho({ flujoPorFiltro: flujoEf, cantidad });
}